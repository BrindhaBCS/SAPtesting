...
System Logon
    ...
    Sleep    2
    Connect To Session
    Open Connection    ${symvar('MASTER_SAP_connection2')}    
    Input Text    wnd[0]/usr/txtRSYST-MANDT    ${symvar('MASTER_Client_Id2')}
    Input Text    wnd[0]/usr/txtRSYST-BNAME    ${symvar('MASTER_User_Name2')}    
    # Input Password   wnd[0]/usr/pwdRSYST-BCODE    ${symvar('MASTER_User_Password')}
    Input Password   wnd[0]/usr/pwdRSYST-BCODE    %{SAP_PASSWORD2}
    Send Vkey    0
    Sleep    5

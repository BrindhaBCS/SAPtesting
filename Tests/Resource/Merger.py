...
class Merger():
    ...
    def create_pdf(symphony_job_id, screenshot_directory=None):
        ...
        # Search upwards for the 'Reports' directory, specific to symphony_job_id
        reports_dir = find_reports_directory(base_dir, symphony_job_id)
...
def find_reports_directory(start_dir, symphony_job_id):
    """
    Recursively search upwards from start_dir for the 'Reports_$job_id' directory.
    Return the path to 'Reports_$job_id' directory if found, else return None.
    """
    current_dir = start_dir
    reports_dir_name = f"Reports_{symphony_job_id}"  # Directory with job ID in its name

    while True:
        reports_dir = os.path.join(current_dir, reports_dir_name)
...
if __name__ == "__main__":
    ...
    screenshot_directory = r'C:\RobotFramework\sap_testing\Reports'  # Ensure this is your screenshot directory

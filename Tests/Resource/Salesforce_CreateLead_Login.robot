...
Create_Lead_login
    ...
    IF    ${mobile_exists} and ${email_exists}
        ...
        Log To Console    **gbStart**copilot_status_a**splitKeyValue**${duplicate_entry}**gbEnd**
        Fail    Duplicate entries detected!
    ELSE IF    ${mobile_exists}
        ${duplicate_entry}    Log    Mobile number ${symvar('mobile_no')} already exists.
        Log To Console    **gbStart**copilot_status_b**splitKeyValue**${duplicate_entry}**gbEnd**
        Fail    Duplicate mobile number detected!
    ELSE IF    ${email_exists}
        ${duplicate_entry}    Log    Email ID ${symvar('email_id')} already exists.
        Log To Console    **gbStart**copilot_status_c**splitKeyValue**${duplicate_entry}**gbEnd**
    ...
    IF  ${is_valid_mobile_no}  
        ...
        Log To Console    **gbStart**copilot_status_e**splitKeyValue**${log_message}**gbEnd**
        Input Text    xpath:(//label[contains(.,'*Phone')]/following::input)    ${mobile}
    ELSE  
        ${log_message}=  Set Variable  Mobile number is invalid: ${mobile}
        Log  message=${log_message}  formatter=repr       
        Log To Console    **gbStart**copilot_status_f**splitKeyValue**${log_message}**gbEnd**  
        IF  not "${mobile}".isdigit()  
            ${log_message}=  Set Variable  Mobile number should contain only digits
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_g**splitKeyValue**${log_message}**gbEnd** 
        ELSE IF  len("${mobile}") < 10  
            ${log_message}=  Set Variable  Mobile number is too short (less than 10 digits)
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_h**splitKeyValue**${log_message}**gbEnd** 
        ELSE IF  len("${mobile}") > 10  
            ${log_message}=  Set Variable  Mobile number is too long (more than 10 digits)
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_i**splitKeyValue**${log_message}**gbEnd** 
        ELSE  
            ${log_message}=  Set Variable  Mobile number is invalid for unknown reasons
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_j**splitKeyValue**${log_message}**gbEnd** 
        END  
        Log  message=${log_message}  formatter=repr  
        Log To Console    **gbStart**copilot_status_k**splitKeyValue**${log_message}**gbEnd** 
    END  
    Sleep    2
    ...
    ELSE  
        ...
        Log  message=${log_message}  formatter=repr  
        Log To Console    **gbStart**copilot_status_l**splitKeyValue**${log_message}**gbEnd** 
        IF  "@" not in "${email}"  
            ${log_message}=  Set Variable  Email is invalid: Missing @ symbol
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_m**splitKeyValue**${log_message}**gbEnd** 
        ELSE IF  "${email}".startswith("@")  
            ${log_message}=  Set Variable  Email is invalid: Missing username
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_n**splitKeyValue**${log_message}**gbEnd** 
        ELSE IF  "${email}".endswith("@")  
            ${log_message}=  Set Variable  Email is invalid: Missing domain
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_o**splitKeyValue**${log_message}**gbEnd** 
        ELSE IF  "." not in "${email.split('@')[-1]}"  
            ${log_message}=  Set Variable  Email is invalid: Missing top-level domain
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_p**splitKeyValue**${log_message}**gbEnd** 
        ELSE  
            ${log_message}=  Set Variable  Email is invalid: Unknown format issue
            Log  message=${log_message}  formatter=repr  
            Log To Console    **gbStart**copilot_status_q**splitKeyValue**${log_message}**gbEnd** 
        END  
        Log  message=${log_message}  formatter=repr  
        Log To Console    **gbStart**copilot_status_r**splitKeyValue**${log_message}**gbEnd** 
    END      
    ...
    IF    '${lead_status}' != 'Open'    
        Input Text    xpath://textarea[@class='slds-textarea']    ${symvar("add_comment")}
        Sleep    2
    ...
    IF  ${is_valid_mobile_no} and ${is_valid_email}  
        ...
        Sleep    2
        Log To Console    **gbStart**copilot_status_t**splitKeyValue**Lead for ${symvar('lastname')} created successfully...**gbEnd**
        Sleep    10
    ...
    ELSE  
        ...
        Log To Console    **gbStart**copilot_status_u**splitKeyValue**Cannot create lead. Invalid Mobile or Email.**gbEnd**
    END

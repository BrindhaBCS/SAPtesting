...
Library    String
*** Variables ***
${filename}    ${symvar('Customer_filename2')}
${sheetname}    ${symvar('Customer_sheetname2')}
${START_ROW}    6
*** Keywords ***
System Logon
    ...
    Sleep    2
    Connect To Session
    Open Connection    ${symvar('Customer_Connection2')}
    Sleep   1
    Input Text    wnd[0]/usr/txtRSYST-MANDT     ${symvar('Customer_Client_Id2')}
    Input Text    wnd[0]/usr/txtRSYST-BNAME    ${symvar('Customer_User_Name2')}
    # Input Password   wnd[0]/usr/pwdRSYST-BCODE    ${symvar('Customer_User_Password')}
    Input Password    wnd[0]/usr/pwdRSYST-BCODE    %{Customer_User_Password2}
    Send Vkey    0
    Sleep    2

...
Material_master   
    ...
    ${passed_rows}    Create List
    ${failed_rows}    Create List
    ...
    FOR    ${initial_row}    IN RANGE    2    ${rows}
        ...
        IF    '${window_exist}' == 'True'
            ...
            Log To Console    ${popup_value}
            Write Excel    ${symvar('Excel_file')}    ${symvar('Sheet_name')}    ${initial_row}    21    ${popup_value}
            Write Excel    ${symvar('Excel_file')}    ${symvar('Sheet_name')}    ${initial_row}    22    Failed
            Close Window    wnd[2]
            Close Window    wnd[1]
            Continue For Loop
        ELSE 
        ...
        Log To Console    ${material_status}
    ...
    Log To Console    Pass count: ${pass_count}
    Log To Console    Fail count: ${fail_count}
    Log To Console    **gbStart**Copilot_Status**splitKeyValue**${pass_count} rows are Passed & ${fail_count} rows are Failed**gbEnd**
